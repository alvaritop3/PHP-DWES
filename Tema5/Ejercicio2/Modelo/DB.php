...
class DB {
    ...
    public static function obtieneProducto($cod_prod) {
     ...
     //Aquí hacemos el método de obtieneTelevisor(codigo del producto)
    public static function obtieneTelevisor($cod_prod) {
        //Esta es la array que le introduzco a ejecuta_consulta
        $array = [':cod' => $cod_prod];
        try {
            $consulta_televisor = self::ejecuta_consulta('SELECT * FROM producto INNER JOIN televisor USING (cod) WHERE cod =:cod', $array);
            if ($consulta_televisor->rowCount() == 1) {
                $televisor = new Televisor($consulta_televisor->fetch());
            }
